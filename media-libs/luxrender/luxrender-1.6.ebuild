...
src_prepare() {
	...
	epatch "${FILESDIR}/${PN}_context_throw_on_error.patch"
	epatch "${FILESDIR}/${PN}_devices_throw_on_error.patch"

	grep -lr "${S}/src/luxrays/core/context.cpp" -e "assert (\!\?started);" | xargs sed -i -f "${FILESDIR}/luxrays_context_assert_started_removal.sed"
	grep -lr "${S}/src/luxrays" -e "assert (\!\?started);" | xargs sed -i -f "${FILESDIR}/luxrays_assert_started_removal.sed"
	grep -lr "${S}/src/slg" -e "assert (\!\?started);" | xargs sed -i -f "${FILESDIR}/slg_assert_started_removal.sed"
	grep -lr "${S}/src/luxcore" -e "assert (\!\?started);" | xargs sed -i -f "${FILESDIR}/luxcore_assert_started_removal.sed"
